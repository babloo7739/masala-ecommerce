...
//   const [address, setAddress] = useState({
//     name: userInfo?.name || "", phone: "", street: "",
//     city: "", state: "", pincode: "",
//   });
//   const [loading, setLoading] = useState(false);
//   const [step, setStep] = useState(1); // 1=address, 2=review
...
//           name: i.name, quantity: i.qty, image: i.image,
...
//               style={{ width: 32, height: 32, background: step > i ? "#b5451b" : step === i + 1 ? "#b5451b" : "#ddd", color: "white", fontSize: 14 }}>
//               {step > i + 1 ? "✓" : i + 1}
//             </div>
//             <span className={`fw-semibold small ${step === i + 1 ? "" : "text-muted"}`}>{s}</span>
//             {i === 0 && <span className="text-muted">→</span>}
...
//           {step === 1 && (
//             <div className="card border-0 shadow-sm p-4" style={{ borderRadius: 12 }}>
//               <h5 className="fw-bold mb-4"><FaMapMarkerAlt className="me-2" color="#b5451b" />Delivery Address</h5>
//               <div className="row g-3">
//                 {[
//                   { name: "name", label: "Full Name", col: 6 },
//                   { name: "phone", label: "Phone Number", col: 6 },
//                   { name: "street", label: "Street Address", col: 12 },
...
//                     <input
//                       name={f.name} className="form-control" value={address[f.name]}
//                       onChange={handleChange} required placeholder={f.label}
//                       style={{ borderRadius: 8 }}
//                     />
//                   </div>
//                 ))}
...
//           {step === 2 && (
//             <div className="card border-0 shadow-sm p-4" style={{ borderRadius: 12 }}>
//               <h5 className="fw-bold mb-3"><FaCheckCircle className="me-2" color="#b5451b" />Review Order</h5>
              
//               <div className="mb-3 p-3 rounded" style={{ background: "#fff8f5" }}>
//                 <h6 className="fw-bold">Delivery to:</h6>
//                 <p className="mb-0">{address.name} | {address.phone}</p>
//                 <p className="mb-0 text-muted small">{address.street}, {address.city}, {address.state} - {address.pincode}</p>
//                 <button className="btn btn-link btn-sm p-0 mt-1" style={{ color: "#b5451b" }} onClick={() => setStep(1)}>Edit Address</button>
//               </div>
...
//                     style={{ width: 50, height: 50, objectFit: "cover", borderRadius: 8 }}
//                     onError={e => { e.target.src = "https://images.unsplash.com/photo-1596040033229-a9821ebd058d?w=50&h=50&fit=crop"; }}
//                   />
//                   <div className="flex-grow-1">
//                     <div className="fw-semibold small">{item.name}</div>
//                     <div className="text-muted small">Qty: {item.qty}</div>
//                   </div>
//                   <div className="fw-bold" style={{ color: "#b5451b" }}>₹{item.price * item.qty}</div>
//                 </div>
//               ))}
...
//               >
//                 {loading ? <><span className="spinner-border spinner-border-sm me-2" />Placing Order...</> : "🎉 Place Order (COD)"}
//               </button>
//             </div>
//           )}
//         </div>
...
//             {cartItems.map(i => (
//               <div key={i._id} className="d-flex justify-content-between mb-1 small">
